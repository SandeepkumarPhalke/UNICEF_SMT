Feature: Issuing tab in SMT application for Store Operator user
  ...
  Background: User login to SMT application and navigate to Stock Overview tab
  	...
  	Scenario Outline: Verify user is able to add issuing line and issuing requisition type data and finalize
  		Given User check available stock balance for "<Product>" is visible in Stock Overview
			...
			And   User wait for "5" seconds
			When  User click on "Issue New Stock" button
  	  Then  "Issue New Stock" text is visible
  	  ...
  	  And   User wait for "3" seconds
  	  Then  Updated Stock balance for "<Product>" is visible in Stock Overview for "Issuing" with "<Units>" units
  		Examples:
   ...
   Scenario Outline: Verify user is able to update issuing line and issuing requisition type data before finalizing
  		Given User check available stock balance for "<Product>" is visible in Stock Overview
			And   User click on "Issuing" tab
			And   User wait for "5" seconds
			And   User click on "Issue New Stock" button
  ...
  Scenario Outline: Verify user is able to delete issuing line and delete issuing requisition type data before finalizing
			Given User click on "Issuing" tab
			And   User wait for "5" seconds
			And   User click on "Issue New Stock" button
  	  And   "Issue New Stock" text is visible
  	  And   User select "<Issue Type>" from "Issue Type" in "Issuing" tab
